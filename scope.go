...
func (scope *Scope) New(value interface{}) *Scope {
	return &Scope{db: scope.NewDB(), Search: &search{}, Value: value, ctx: context.Background()}
}
...
func (scope *Scope) Trace(t time.Time) {
	...
	span := opentracing.SpanFromContext(scope.ctx)
	if span == nil {
		fmt.Printf("SpanFromContext err")
		return
	}
	span.SetTag("sql", scope.Sql)
	span.SetTag("time", fmt.Sprintf("%.2fms", float64(NowFunc().Sub(t).Nanoseconds()/1e4)/100.0))
	span.Finish()
}
