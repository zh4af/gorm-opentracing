...
func (association *Association) Delete(values ...interface{}) *Association {
	...
	if len(primaryKeys) == 0 {
		...
		if relationship.Kind == "many_to_many" {
			...
			if err := relationship.JoinTableHandler.Delete(scope.ctx, query, relationship); err == nil {
...
func (association *Association) Replace(values ...interface{}) *Association {
	...
	if relationship.Kind == "many_to_many" {
		...
		association.setErr(relationship.JoinTableHandler.Delete(scope.ctx, query, relationship))
	} else {
...
func (association *Association) Count() int {
	...
	if relationship.Kind == "many_to_many" {
		relationship.JoinTableHandler.JoinWith(scope.ctx, scope.NewDB(), association.Scope.Value).Table(newScope.TableName()).Count(scope.ctx, &count)
		...
		countScope.Count(scope.ctx, &count)
	} else if relationship.Kind == "belongs_to" {
		if v, ok := scope.FieldByName(association.Column); ok {
			whereSql := fmt.Sprintf("%v.%v = ?", newScope.QuotedTableName(), newScope.Quote(relationship.ForeignDBName))
			scope.DB().Table(newScope.TableName()).Where(whereSql, v).Count(scope.ctx, &count)
		}
	}
