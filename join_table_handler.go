...
type JoinTableHandlerInterface interface {
	...
	Table(db *DB) string
	Add(ctx context.Context, db *DB, source interface{}, destination interface{}) error
	Delete(ctx context.Context, db *DB, sources ...interface{}) error
	JoinWith(ctx context.Context, db *DB, source interface{}) *DB
}
...
func (s JoinTableHandler) GetSearchMap(ctx context.Context, db *DB, sources ...interface{}) map[string]interface{} {
	values := map[string]interface{}{}

	for _, source := range sources {
		scope := db.NewScope(ctx, source)
...
func (s JoinTableHandler) Add(ctx context.Context, db *DB, source1 interface{}, source2 interface{}) error {
	scope := db.NewScope(ctx, "")
	searchMap := s.GetSearchMap(ctx, db, source1, source2)
	...
	return db.Exec(scope.ctx, sql, values...).Error
}

func (s JoinTableHandler) Delete(ctx context.Context, db *DB, sources ...interface{}) error {
	var conditions []string
	var values []interface{}

	for key, value := range s.GetSearchMap(ctx, db, sources...) {
		conditions = append(conditions, fmt.Sprintf("%v = ?", key))
		values = append(values, value)
	}

	return db.Table(s.Table(db)).Where(strings.Join(conditions, " AND "), values...).Delete(ctx, "").Error
}

func (s JoinTableHandler) JoinWith(ctx context.Context, db *DB, source interface{}) *DB {
	quotedTable := s.Table(db)
