...
func (scope *Scope) buildWhereCondition(clause map[string]interface{}) (str string) {
	...
	for _, arg := range args {
		...
		case []int, []int8, []int16, []int32, []int64, []uint, []uint16, []uint32, []uint64, []string, []interface{}:
		...
		if fromField != nil {
			if relationship := fromField.Relationship; relationship != nil {
				if relationship.Kind == "many_to_many" {
					...
					scope.Err(joinTableHandler.JoinWith(scope.ctx, toScope.db, scope.Value).Find(scope.ctx, value).Error)
				} else if relationship.Kind == "belongs_to" {
					sql := fmt.Sprintf("%v = ?", scope.Quote(toScope.PrimaryKey()))
					foreignKeyValue := fromFields[relationship.ForeignDBName].Field.Interface()
					scope.Err(toScope.db.Where(sql, foreignKeyValue).Find(scope.ctx, value).Error)
					...
					scope.Err(query.Find(scope.ctx, value).Error)
				}
			} else {
				sql := fmt.Sprintf("%v = ?", scope.Quote(toScope.PrimaryKey()))
				scope.Err(toScope.db.Where(sql, fromField.Field.Interface()).Find(scope.ctx, value).Error)
			}
			return scope
		} else if toField != nil {
			sql := fmt.Sprintf("%v = ?", scope.Quote(toField.DBName))
			scope.Err(toScope.db.Where(sql, scope.PrimaryKeyValue()).Find(scope.ctx, value).Error)
			return scope
		}
	}
...
func (scope *Scope) createJoinTable(field *StructField) {
	if relationship := field.Relationship; relationship != nil && relationship.JoinTableHandler != nil {
		...
		if !scope.Dialect().HasTable(scope, joinTable) {
			...
			scope.Err(scope.NewDB().Exec(scope.ctx, fmt.Sprintf("CREATE TABLE %v (%v)", scope.Quote(joinTable), strings.Join(sqlTypes, ","))).Error)
		}
		scope.NewDB().Table(joinTable).AutoMigrate(scope.ctx, joinTableHandler)
	}
}
