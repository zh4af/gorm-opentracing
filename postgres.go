...
func (postgres) HasTable(scope *Scope, tableName string) bool {
	var count int
	scope.NewDB().Raw("SELECT count(*) FROM INFORMATION_SCHEMA.tables WHERE table_name = ? AND table_type = 'BASE TABLE'", tableName).Row(scope.ctx).Scan(&count)
	return count > 0
}

func (postgres) HasColumn(scope *Scope, tableName string, columnName string) bool {
	var count int
	scope.NewDB().Raw("SELECT count(*) FROM INFORMATION_SCHEMA.columns WHERE table_name = ? AND column_name = ?", tableName, columnName).Row(scope.ctx).Scan(&count)
	return count > 0
}

func (postgres) RemoveIndex(scope *Scope, indexName string) {
	scope.NewDB().Exec(scope.ctx, fmt.Sprintf("DROP INDEX %v", indexName))
}

func (postgres) HasIndex(scope *Scope, tableName string, indexName string) bool {
	var count int
	scope.NewDB().Raw("SELECT count(*) FROM pg_indexes WHERE tablename = ? AND indexname = ?", tableName, indexName).Row(scope.ctx).Scan(&count)
	return count > 0
}
